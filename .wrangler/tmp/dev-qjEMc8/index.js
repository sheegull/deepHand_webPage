...
// .wrangler/tmp/bundle-YQ6U4m/strip-cf-connecting-ip-header.js
...
var rateLimit = /* @__PURE__ */ __name(() => {
  return async (c, next) => {
    ...
    const ip = c.req.headers.get("CF-Connecting-IP") || "development";
...
var sanitizeInput = /* @__PURE__ */ __name((data) => {
  const sanitized = {};
...
var requestSchema = z.object({
  ...
  organization: z.string().max(100).optional(),
  email: z.string().email("Invalid email address"),
  backgroundPurpose: z.string().min(1, "Background and purpose is required").max(1e3),
  dataType: z.array(z.string()).min(1, "At least one data type must be selected"),
  dataDetails: z.string().max(1e3).optional(),
  ...
  otherRequirements: z.string().max(1e3).optional()
});
var validateContactForm = /* @__PURE__ */ __name((data) => {
  const result = contactSchema.safeParse(data);
  return result;
}, "validateContactForm");
var validateRequestForm = /* @__PURE__ */ __name((data) => {
...
app.post("/api/contact", async (c) => {
  try {
    ...
    const { success, data: valid, error: error3 } = validateContactForm(sanitized);
    if (!success) {
      return c.json({ error: "Validation failed", details: error3 }, 400);
    }
    if (c.env.NOTIFY) {
      try {
        const message = {
...
app.post("/api/request-data", async (c) => {
  try {
    const data = await c.req.json();
    const sanitized = sanitizeInput(data);
    const { success, data: valid, error: error3 } = validateRequestForm(sanitized);
    if (!success) {
      return c.json({ error: "Validation failed", details: error3 }, 400);
    }
...
\u305D\u306E\u4ED6\u3001\u8A73\u7D30\u3084\u3054\u8981\u671B: ${valid.otherRequirements || "N/A"}`
      ...
      console.log("Email would be sent:", {
        ...
        data: {
          ...
          organization: valid.organization,
          email: valid.email,
          backgroundPurpose: valid.backgroundPurpose,
          dataType: dataTypes,
          dataDetails: valid.dataDetails,
          dataVolume: valid.dataVolume,
          deadline: valid.deadline,
          budget: valid.budget,
          otherRequirements: valid.otherRequirements
        }
      });
    }
    if (c.env.FORM_STORAGE) {
...
// .wrangler/tmp/bundle-YQ6U4m/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-YQ6U4m/middleware-loader.entry.ts
